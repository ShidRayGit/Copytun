  ...
  "nodes": [
    {
      "name": "listener",
      "type": "TcpListener",
      "settings": { "address": "0.0.0.0", "port": ${local_port} },
    ...
    },
    {
      "name": "connector",
      "type": "TcpConnector",
      "settings": { "address": "${foreign_ip}", "port": ${tunnel_port} }
    }
  ]
}
EOF
  ...
  "nodes": [
    {
      "name": "listener",
      "type": "TcpListener",
      "settings": { "address": "0.0.0.0", "port": ${tunnel_port} },
    ...
    },
    {
      "name": "connector",
      "type": "TcpConnector",
      "settings": { "address": "127.0.0.1", "port": ${target_port} }
    }
  ]
}
EOF
...
install_waterwall() {
  ...
  local found_bin=""
  found_bin="$(find "${tmpd}/ww" -maxdepth 4 -type f -name 'WaterWall' 2>/dev/null | head -n 1 || true)"
  [[ -n "$found_bin" ]] || die "Could not find WaterWall binary inside the zip."
  install -m 0755 "$found_bin" "${WW_BIN}"

  local found_libs=""
  found_libs="$(find "${tmpd}/ww" -maxdepth 4 -type d -name 'libs' 2>/dev/null | head -n 1 || true)"
...
create_tunnel() {
  ...
  if [[ "$role_choice" == "1" ]]; then
    ...
    write_core_json "$tdir" "$tn" "configs/iran-halfduplex.json"
    write_meta_env "$tdir" \
      "TUNNEL_NAME=${tn}" \
      "ROLE=iran" \
    ...
    write_core_json "$tdir" "$tn" "configs/foreign-halfduplex.json"
    write_meta_env "$tdir" \
      "TUNNEL_NAME=${tn}" \
      "ROLE=foreign" \
...
view_log_files() {
  ...
  case "$mode" in
    1)
      tail -n 200 "${ldir}/${selected}" > /dev/tty
      printf -- "\n" > /dev/tty
      tty_pause
      ;;
    2)
