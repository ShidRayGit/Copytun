...
# - Multi-tunnel
# - Persistent via systemd (survives logout/reboot)
# - Installs WaterWall v1.41 (amd64/arm64 + old-cpu)
#
# Run:
#   sudo ./copytun.sh
# One-liner:
...
restart_tunnel() {
  local tname="$1"
  systemctl restart "$(service_name_for "$tname")"
}

stop_tunnel() {
  local tname="$1"
  systemctl stop "$(service_name_for "$tname")" || true
}
