...
clean_input() {
  # removes Windows CR and trims all whitespace
  local s="${1:-}"
  s="${s//$'\r'/}"
  s="${s//[[:space:]]/}"
  printf '%s' "$s"
...
require_systemd() {
  ...
  [[ -d /run/systemd/system ]] || die "systemd does not seem to be running on this host."
}
...
# --- input helpers (single-key menus) ---
read_menu_choice_1key() {
  local prompt="$1"
  local c
  read -r -n 1 -p "$prompt" c || true
  echo
  c="$(clean_input "$c")"
  printf '%s' "$c"
}
...
prompt_text() {
  ...
  # do not remove dots/colons etc; only strip CR
  t="${t//$'\r'/}"
  printf '%s' "$t"
}
...
pick_build_menu() {
  ...
  if [[ "$arch" == "x86_64" ]]; then
    ...
    local c
    while true; do
      c="$(read_menu_choice_1key "Choice: ")"
      case "${c:-1}" in
  ...
  cat > "${dir}/${tname}.json" <<JSON
{
  ...
  "misc": { "workers": 0, "mtu": 1500, "ram-profile": "client", "libs-path": "libs/" },
  ...
  cat > "${dir}/${tname}.json" <<JSON
{
  ...
  "misc": { "workers": 0, "mtu": 1500, "ram-profile": "server", "libs-path": "libs/" },
...
create_tunnel() {
  ...
  local role_choice
  role_choice="$(read_menu_choice_1key "Choice (1/2): ")"
